...
void attachInterrupt(uint32_t pin, void (*callback)(void), uint8_t mode)
{
	...
	uint8_t pio = digitalPinToPort(pin);
	...
	if(mode<2){ //check if low or high.
		Port->PCR[pos] |= PORT_PCR_IRQC((mode<<2)|0x08); //move mode LSB to mux
		// relevant position selecting low/high, and add 1000 for compliance.
	}
	else if(mode&0x80){//verify type is interrupt define
		Port->PCR[pos] |= PORT_PCR_IRQC((mode&0x0F)); //filter out IRQ setting
	}
